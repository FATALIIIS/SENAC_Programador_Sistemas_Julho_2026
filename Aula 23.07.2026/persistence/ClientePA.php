...
class ClientePA{
	...
	public function buscarPorCod($cod_cli)
	{
		...
		$consulta=$this->con->consultar($sql,[$cod_cli],'i');
		$this->con->desconectar();
		return $consulta;
	}
	public function alterar($cliente)
	{
		$sql="UPDATE cliente SET nome=?,endereco=?,telefone=?,data_nasci=?,cpf=?,login=?,senha=?";
		$campos=[
			$cliente->getNome(),
			$cliente->getEndereco(),
			$cliente->getTelefone(),
		...
		$resposta=$this->con->executar($sql,$campos,'sssssssi');
		$this->con->desconectar();
		return $resposta;
	}
	public function excluir($cod_cli)
	{
		...
		$sql="DELETE FROM pet WHERE cod_cli=?";
		$resposta=$this->con->executar($sql,[$cod_cli],'i');
		if (!$resposta) {
			$this->con->desconectar();
			return false;
		}else{
